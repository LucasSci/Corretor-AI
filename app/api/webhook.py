...
def _cleanup_recent_outgoing(now_ts: float) -> None:
    ttl: int = max(1, settings.WEBHOOK_LOOP_GUARD_TTL_SEC)
...
def _is_recent_outgoing(remote_jid: str, text: str) -> bool:
    ...
    if ts is None:
        return False
    return (now_ts - ts) <= max(1, settings.WEBHOOK_LOOP_GUARD_TTL_SEC)
...
async def chat(payload: MessageIn) -> Dict[str, Any]:
    # Placeholder for older route compatibility if needed
    context: str = await ai_service.get_context_from_db(payload.text)
    ai_response: str = await ai_service.generate_response(payload.text, context)
    return {"contact_id": payload.contact_id, "reply": ai_response}
